...
const Sidebar: React.FC<SidebarProps> = ({ userName, isMobileView }) => {
  ...
  return (
    <div
      className={`bg-[#AAFFA9] bg-opacity-5 text-white w-full md:w-64 flex flex-col ${
        isMobileView ? "md:fixed md:left-0 md:top-0" : ""
      } md:h-full md:border-r border-gray-700 mt-4 md:mt-0`}
    >
      {/* Company Name */}
      ...
      {/* Profile Section */}
      <div
        className={`p-2 flex items-center cursor-pointer rounded-lg mt-3 ${
          isDropdownOpen ? "bg-[#FFFFFF] bg-opacity-10" : "bg-transparent"
        }`}
      ...
      <ul className="py-2">
        ...
        <li className="px-2 py-1 flex flex-row cursor-pointer group text-white">
          ...
          <span className="group-hover:text-[#AAFFA9]">
      ...
      <div className="p-2 flex items-center">
        ...
        <button className="bg-transparent text-purple-600">Logout</button>
      </div>
    </div>
  );
};
